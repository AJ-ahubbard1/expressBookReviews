...
public_users.get('/', async (req, res) => {
    try {
        res.status(200).send(await JSON.stringify(books, null, 4))
    }
    catch (err) {
...
public_users.get('/isbn/:isbn', async (req, res) => {
  try {
    ...
    res.status(200).send(await JSON.stringify(book, null, 4));
  }
  catch (err) {
    res.status(500).send(err)
  }
...
public_users.get('/author/:author', async (req, res) => {
  ...
  try {
    ...
    res.status(200).send(await JSON.stringify(booksByAuthor, null, 4))
  }
  catch (err) {
    res.status(500).send(err)
  }
...
public_users.get('/title/:title', async (req, res) => {
  try {
    ...
    res.status(200).send(await JSON.stringify(booksByTitle, null, 4))
  }
  catch (err) {
    res.status(500).send(err)
  }
